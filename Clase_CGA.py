...
def GeneratePopulation(M,N,Limits):
    ...
    for i in range(M):      
        for j in range(N):
            if j>=0 and j<4:
                Chromo[j]=rand.randint(0,1)
            if j==4 or j==8:
                Chromo[j]=(Limits['UB'][j] - Limits['LB'][j]) * rand.random() + Limits['LB'][j]
            if j==5:
                Chromo[j]=rand.uniform(Chromo[j-1]-1,Limits['UB'][j])# *****
            if j==6 or j==7:
                # Chromo[j]=(Limits['UB'][j] - Limits['LB'][j]) * rand.uniform(Chromo[j-1]-4,Chromo[j-1]+4) + Limits['LB'][j]
                Chromo[j]=rand.uniform(Chromo[j-1]-4,Chromo[j-1]+2)# ***** Esta construcción se debe a la forma en la que debe lucir la curva de los angulos
                if Chromo[j]>Limits['UB'][j]:
                    Chromo[j]=Limits['UB'][j]
                elif Chromo[j]<Limits['LB'][j]:
                    Chromo[j]=Limits['LB'][j]
            if j>8:
                Chromo[j]= (Limits['UB'][j] - Limits['LB'][j]) * rand.uniform(Chromo[j-1],1) + Limits['LB'][j]
...
def Mutation(Genes,Limits,Pm):
    ...
    for k in range(N):
        ...
        if k>=0 and k <4:
            if R < Pm:
                Genes[k]=rand.randint(0,1)
        if k>=4 :#and k <=8:
            if R < Pm:
                Genes[k] = (Limits['UB'][k] - Limits['LB'][k] ) * rand.random() + Limits['UB'][k]
        # elif k==9 or k==10:
        #     if R < Pm:
        #         Genes[k] = rand.uniform(Genes[k-1],Limits['UB'][k])
...
def Fitness_Func(Chromo,Airfoils_mat,Convergencia): 
    ...
    return cp.Coef_Potencia.Calculo(Chromo,Airfoils_mat,Convergencia)
